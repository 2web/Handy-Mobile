...
- **No new crates.** Everything listed above is already in `src-tauri/Cargo.toml`. If a task seems to need a new dependency, stop and report instead of adding one.
- **All game code lives in new files.** Edits to existing files are limited to: `src-tauri/src/lib.rs` (module declaration + command registration), `src-tauri/src/settings.rs` (two fields + two default functions), `src/components/Sidebar.tsx` (one entry), `src/stores/settingsStore.ts` (two updater entries), `src/i18n/locales/en/translation.json` and `ru/translation.json`.
...
- **The first `cargo test` build of this project takes 10–20 minutes.** That is normal. Do not interpret it as a hang and do not kill it.
...
| File | Responsibility |
|---|---|
| `src-tauri/src/poe2/mod.rs` | module wiring, re-exports |
| `src-tauri/src/poe2/items.rs` | text → structure; all item regexes |
| `src-tauri/src/poe2/fixtures/sceptre.txt` | real item, embedded via `include_str!` |
| `src-tauri/src/poe2/fixtures/body_armour.txt` | real item, embedded via `include_str!` |
| `src-tauri/src/poe2/store.rs` | `poe2.db`, schema, migrations, reads/writes |
| `src-tauri/src/poe2/clipboard_watch.rs` | polling loop, strict item test |
| `src-tauri/src/poe2/commands.rs` | Tauri commands |
...
Expected: PASS, 44 tests (this task adds no new tests — commands are thin wrappers over already-tested code).
