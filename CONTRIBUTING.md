...
> Полная англоязычная версия — выше. Здесь переведены основные разделы. **Команды, названия веток, URL и conventional-commit префиксы не переводятся** — используйте их как есть.

Спасибо за интерес к развитию Handy! Это руководство поможет начать вносить вклад в опенсорсное приложение для распознавания речи.

### ⚠️ Заморозка фич (feature freeze)

**Сейчас в Handy действует заморозка новых фич.** PR с новой фичей, которую сообщество не запрашивало, будет отклонён. Если фичу просили или вы явно собрали поддержку сообщества — её ещё могут рассмотреть.

**Приоритет №1 — исправление багов.** Открыто 60+ issues. Пожалуйста, сосредоточьтесь на фиксах и стабильности.

### 📖 Философия

Handy стремится быть самым «форкабельным» приложением для распознавания речи: одновременно полезный инструмент и фундамент, на котором строят другие — понятная, простая кодовая база на пользу сообществу. Приоритеты:

- **Простота** — понятный поддерживаемый код важнее «умных» решений
- **Расширяемость** — легко форкать и кастомизировать
- **Приватность** — всё локально и офлайн
- **Доступность** — бесплатный инструмент, доступный каждому
...
- платформенные build-инструменты (см. [BUILD.md](BUILD.md))
...
6. **Запустите в режиме разработки:**
   ```bash
   bun run tauri dev
   # На macOS при ошибках cmake:
   CMAKE_POLICY_VERSION_MINIMUM=3.5 bun run tauri dev
   ```

Подробные платформенные инструкции — в [BUILD.md](BUILD.md).

**Структура кодовой базы:**

Бэкенд (Rust — `src-tauri/src/`):

- `lib.rs` — точка входа приложения, настройка Tauri
- `managers/` — основная бизнес-логика (аудио, модели, транскрипция)
- `audio_toolkit/` — низкоуровневая обработка звука (запись, VAD)
- `commands/` — обработчики Tauri-команд для связи с фронтендом
- `shortcut.rs` — глобальные горячие клавиши
- `settings.rs` — управление настройками

Фронтенд (React/TypeScript — `src/`):
...
Подробнее — в разделе Architecture в [README.md](README.md) или в [AGENTS.md](AGENTS.md).

### 🐛 Сообщения о багах

**Перед отправкой:** поищите в [existing issues](https://github.com/cjpais/Handy/issues) и [discussions](https://github.com/cjpais/Handy/discussions), проверьте на последнем релизе, включите debug-режим (`Cmd/Ctrl+Shift+D`) для диагностики.

**В баг-репорт включите:**

- **Система:** версия приложения, ОС (напр. macOS 14.1 / Windows 11 / Ubuntu 22.04), CPU, GPU
- **Детали бага:** чёткое описание, шаги воспроизведения, ожидаемое и фактическое поведение, скриншоты/логи, данные из debug-режима

Используйте шаблон [Bug Report](.github/ISSUE_TEMPLATE/bug_report.md).

### 💡 Предложение фич

Для фич используются **GitHub Discussions**, а не issues (issues — только для багов и конкретных задач).

Перед предложением поищите в [discussions](https://github.com/cjpais/Handy/discussions). Затем в [Discussions](https://github.com/cjpais/Handy/discussions) → «New discussion» → выберите категорию и опишите: какую проблему решаете, предлагаемое решение, рассмотренные альтернативы, как это вписывается в философию Handy.

### 🔧 Вклад в код

**Перед началом (критично):**

1. **Поищите в существующих issues и PR** — как в открытых, так и в **закрытых**. Возможно, это уже сделано или закрыто по причине.
2. **Если что-то было закрыто ранее** — приведите весомый аргумент, соберите обратную связь в [Discussions](https://github.com/cjpais/Handy/discussions) и сошлитесь на неё в PR.
3. **Соберите фидбэк сообщества для фич** — PR с подтверждённым интересом сообщества **мержатся гораздо охотнее**.
...
1. **Создайте ветку:**
   ...
   git checkout -b fix/your-bug-fix
   ```
2. **Внесите изменения** — чистый поддерживаемый код, следуйте существующему стилю, комментируйте сложную логику, держите коммиты атомарными.
3. **Тщательно протестируйте** — на целевых платформах, проверьте, что старое не сломалось, крайние случаи и ошибки, используйте debug-режим.
4. **Закоммитьте** (conventional commits):
   ```bash
   git add .
   git commit -m "feat: add your feature description"
   ```
   Префиксы: `feat:` (фичи), `fix:` (баги), `docs:` (документация), `refactor:`, `test:`, `chore:`.
5. **Держите форк в актуальном состоянии:**
   ```bash
   git fetch upstream
   git rebase upstream/main
   ```
6. **Запушьте в свой форк:**
   ...
   git push origin feature/your-feature-name
   ```
7. **Создайте Pull Request** в [репозитории Handy](https://github.com/cjpais/Handy), заполните шаблон полностью: описание изменений, ссылки на issues/discussions, **фидбэк сообщества** (особенно для фич), как тестировали, скриншоты/видео, breaking changes.

   **Помните:** PR с поддержкой сообщества в приоритете. Собрать фидбэк не обязательно, но это заметно ускоряет мерж.

**Раскрытие использования ИИ:** PR с помощью ИИ приветствуются — просто укажите в описании: использовался ли ИИ (да/нет), какие инструменты (напр. «Claude Code», «GitHub Copilot», «ChatGPT») и насколько активно (напр. «сгенерировал boilerplate», «помог с отладкой», «написал большую часть кода»).

**Стиль кода:**

- **Rust:** `cargo fmt`, `cargo clippy` без предупреждений, описательные имена, doc-комментарии для публичных API, явная обработка ошибок (без `unwrap` в проде)
- **TypeScript/React:** строгий TypeScript без `any`, best practices хуков, функциональные компоненты, маленькие сфокусированные компоненты, Tailwind CSS
- **Общее:** самодокументируемый код, комментарии для неочевидной логики, маленькие функции с одной задачей, читаемость важнее «умности»

**Тестирование:** ручное — `bun run tauri dev` с debug-режимом, разные платформы и аудиоустройства; production — `bun run tauri build`.

### 📝 Вклад в документацию

Улучшения документации очень ценятся: правки README.md, BUILD.md, этого CONTRIBUTING.md, комментарии/doc-комментарии, туториалы и гайды, улучшение сообщений об ошибках, контент сайта проекта.

### 🤝 Правила сообщества

- **Уважительно и инклюзивно** — рады участникам любого уровня
- **Терпеливо** — проект ведёт небольшая команда, ответ может занять время
- **Конструктивно** — фокус на решениях
- **В сотрудничестве** — помогайте другим и делитесь знаниями
- **Сначала поиск** — проверьте existing issues/discussions

### 🎯 Хорошие первые задачи

Ищите issues с метками `good first issue` или `help wanted` — они чётко описаны, хороши для знакомства с кодом, с поддержкой ментора.

### 📞 Где получить помощь

- **Discord:** [сообщество в Discord](https://discord.com/invite/WVBeWsNXK4)
...
Внося вклад в Handy, вы соглашаетесь, что он будет лицензирован под MIT. См. [LICENSE](LICENSE).

---

**Спасибо за вклад в Handy!** Ваши усилия делают технологию распознавания речи доступнее, приватнее и расширяемее для всех.
