...
/// network. Managed state; mirrors the `TrayState` pattern.
...
struct MenuInputs {
    ...
    /// Translation target shown as checked in the language submenu.
    translation_language: String,
    /// Post-processing model checked in the model submenu.
    post_process_model: String,
    /// Cached model names for the active post-processing provider.
    post_process_models: Vec<String>,
}
...
fn compute_desired(app: &AppHandle, icon_state: TrayIconState) -> TrayDesired {
    ...
    let post_process_model = settings
        .post_process_models
        .get(&settings.post_process_provider_id)
        .cloned()
        .unwrap_or_default();
    // The cache is managed state like the tray itself; an early sync that
    // happens before it exists just shows an empty model list.
    let post_process_models = app
        .try_state::<PostProcessModelCache>()
        .map(|cache| cache.get())
        .unwrap_or_default();
    ...
    TrayDesired {
        ...
        menu: MenuInputs {
            ...
            translation_language: settings.translation_target_language,
            post_process_model,
            post_process_models,
        },
    }
...
fn build_menu(app: &AppHandle, inputs: &MenuInputs) -> tauri::Result<(Menu<tauri::Wry>, String)> {
    ...
    let menu = if inputs.busy {
        ...
        // Quick switch for the translation target. Built from `inputs` like
        // everything else here, so the menu stays a pure function of the
        // snapshot and the diff in `apply_on_main` stays meaningful.
        let language_submenu = Submenu::with_id(
            app,
        ...
        for (code, is_active) in
            checked_entries(TRANSLATION_LANGUAGE_SHORTLIST, &inputs.translation_language)
        {
        ...
        // Post-processing model picker. The list comes from the cache filled by
        // `refresh_post_process_models`, so no network call happens here.
        let pp_model_submenu =
            Submenu::with_id(app, "pp_model_submenu", &strings.post_process_model, true)?;
        for (model, is_active) in
            checked_entries(&inputs.post_process_models, &inputs.post_process_model)
        {
            if model.is_empty() {
                continue;
            }
            ...
            let item =
                CheckMenuItem::with_id(app, &item_id, &model, true, is_active, None::<&str>)?;
            pp_model_submenu.append(&item)?;
        }
        let refresh_models_i = MenuItem::with_id(
            app,
            "pp_model_refresh",
        ...
        pp_model_submenu.append(&separator()?)?;
        pp_model_submenu.append(&refresh_models_i)?;
...
mod tests {
    use super::{
        checked_entries, last_transcript_text, load_tray_icon, MenuInputs, TrayDesired,
        TrayIconState, TRANSLATION_LANGUAGE_SHORTLIST,
    };
